...
public record SaveAcademicRecordRequest(
    ...
    @Min(value = 0) 
    @Max(value = 100) 
    @Schema(title = "Attendance", example = "85") 
    Integer attendance,

    @NotNull(message = "A nota final é obrigatória") 
    @PositiveOrZero 
    @Max(value = 10) 
    @Schema(title = "Final Grade", example = "9.5") 
    Double finalGrade,

    @NotNull(message = "O semestre é obrigatório") 
    @Schema(title = "Semester", example = "2023/1") 
    String semester,
    ...
    @Schema(title = "Observation", example = "Excelente aluno") 
    String obs
) {}
